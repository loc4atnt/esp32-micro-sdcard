...
void SDClass::ls(const char* filepath, uint8_t indent) {
  printDirectory(open(filepath), indent);
}
...
int16_t File::fgets(char* str, int16_t num, char* delim ) {
  char ch;
  int16_t n = 0;
  int16_t r = -1;
  while ((n + 1) < num && (r = _file->read(&ch, 1)) == 1) {
    // delete CR
    if (ch == '\r') {
      continue;
    }
